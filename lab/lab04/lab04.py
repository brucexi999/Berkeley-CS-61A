...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1, amount//price+1):  # This loop explore all possible branches at a particular node.
                # Hint: The display function will help you add fruit to the cart.
                new_cart = cart + display(fruit, k)
                add(remaining_fruits, amount - k * price, new_cart)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    ref_point = make_city("ref", lat, lon)
    distance_a = distance(ref_point, city_a)
    distance_b = distance(ref_point, city_b)
