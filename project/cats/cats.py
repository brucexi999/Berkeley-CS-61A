...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    filtered_paragraphs = [i for i in paragraphs if select(i)]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_list = [diff_function(typed_word, word_list[i], limit) for i in range(len(word_list))]
        smallest_diff = min(diff_list)
        if smallest_diff > limit:
            return typed_word
        else:
            return word_list[diff_list.index(smallest_diff)]
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def recursion(current_diff, typed_str, source_str):
        if current_diff > limit:
            return current_diff
        elif current_diff <= limit:
            increment = int(typed_str[0] != source_str[0])
            if (len(typed_str)) == 1 and (len(source_str)) == 1:
                return current_diff+increment
            elif (len(typed_str)) == 1 and (len(source_str)) != 1:
                return current_diff+len(source_str[1:])+increment
            elif (len(typed_str)) != 1 and (len(source_str)) == 1:
                return current_diff+len(typed_str[1:])+increment
            else:
                return recursion(current_diff+increment, typed_str[1:], source_str[1:])

    return recursion(0, typed, source)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    # If one of the strings is empty, the edit distance is the length of the other string because we
    # need that many insertions or deletions to match the empty string.
    if typed == "" or source == "":
        return max(len(typed), len(source))
    # If the limit is 0, no edits are allowed. If start and goal are different, the cost is 1; otherwise, it is 0.
    elif limit == 0:
        return int(typed != source)
    # If start and goal are already identical, the edit distance is 0.
    elif typed == source:
        return 0
    # If the first characters of start and goal are the same, we skip them and recursively compute the distance
